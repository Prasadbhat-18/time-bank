...
function AppContent() {
  ...
  // Initialize unread count with new chat system
  useEffect(() => {
    if (user?.id) {
      // Import and use the new chat service
      import('./services/groqChatService').then(({ groqChatService }) => {
        const updateUnreadCount = () => {
          const totalUnread = groqChatService.getTotalUnreadCount(user.id);
        ...
        const unsubscribe = groqChatService.subscribeToNotifications(user.id, () => {
          updateUnreadCount();
        });
    ...
    // Listen for switch to chat view events
    const handleSwitchToChat = () => {
      setCurrentView('chat');
    };

    window.addEventListener('timebank:switchToChat', handleSwitchToChat);
  ...
  {currentView === 'chat' && <ComprehensiveChatView />}
      </main>
